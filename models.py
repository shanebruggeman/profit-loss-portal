...
position_watches = db.Table('position_watches',
                            db.Column('position_id', db.Integer,
                                      db.ForeignKey('stock_positions.stock_position_id', ondelete='cascade',
                                                    onupdate='cascade')),
...
class Transaction(db.Model):
    ...
    account_id = db.Column(db.Integer,
                           db.ForeignKey("accounts.account_id", ondelete='CASCADE'))  # may need re-evaluated
    exchange_id = db.Column(db.Integer,
                            db.ForeignKey("exchanges.exchange_id", ondelete='CASCADE'))  # may need re-evaluated
    ...
    def clone(self):
        copy = Transaction(self.account_id, self.exchange_id, self.price, self.units, self.sec_sym, self.settle,
                           self.entry, self.trade, self.ticket_number, self.buy_sell, self.commission, self.isPosition)
    ...
    def __repr__(self):
        return '<Transaction id="{}" symbol="{}" buy_sell={} isPosition={} units="{}" price="{}" commission="{}" ' \
               'date="{}" account_id="{}">'.format(self.transaction_id, self.sec_sym, self.buy_sell,
                                                   self.isPosition, self.units, self.price,
                                                   self.commission, self.settle.date(), self.account_id)
...
class StockPosition(db.Model):
    ...
    def get_close(self):
        ...
        print 'found these close transactions'
        print close_transactions
    ...
    def __repr__(self):
        ...
        rep = '<StockPosition id={} date={} stock={} account_id={}>'.format(self.stock_position_id, self.date, self.symbol, self.account_id)
