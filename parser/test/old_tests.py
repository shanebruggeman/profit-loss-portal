...
class Test(object):
	...
	# test_maketake_file -> maketake filename
	# test_parse_file -> testdata filename

	def __init__(self, testdata, maketake):
		self.set_testdata_file(testdata)
		self.set_maketake_file(maketake)

	def set_testdata_file(self, filename):
		self.test_parse_file = filename

	def set_maketake_file(self, filename):
		self.test_maketake_file = filename

	def evaluate(self):
		parse_transactions(self.test_parse_file, self.test_maketake_file)
...
class TestRunner(object):
	...
	def run_tests(self):
		trial = Test(testdata_file, test_maketake_file)
		self.trial_result = trial.evaluate()

		test_results1 = test_set1(self.trial_result)

		assert(test_results1)

	def test_set1(self, result):
		# contains only single orders
		single_orders_only = False
...
# 	hbr_str= [{'Commission': '-1.00',
# 			   'EncryptMethod': '0', 
# 			   'BodyLength': '084', 
# 			   'SenderCompID': '_SenderCompID', 
# 			   'TargetCompID': '_TargetCompID', 
# 			   'MsgSeqNum': '6255', 'BeginString': 
# 			   'FIX.4.2', 'MsgType': 'A', 
# 			   'CheckSum': '142', 
# 			   'SendingTime': '20151006-02:52:50.765', 
# 			   'HeartBtInt': '30'}, 
			   
# 			   {'Commission': '-1.00', 
# 			    'CheckSum': '148','BodyLength': 
# 			    '055', 'SenderCompID': 'XXXX', 
# 			    'TargetCompID': 'YYYY', 
# 			    'MsgSeqNum': '55006', 
# 			    'BeginString': 'FIX.4.2', 
# 			    'MsgType': '0', 
# 			    'SendingTime': '20151006-02:52:57.702'}]
