...
def get_transactions_for_date(account, date):
    ...
    if date == "today":
        ...
        positionList = StockPosition.query.filter(StockPosition.account_id == account,
                                                  StockPosition.date < begin_today).all()
    # transactionList = db.session.query(Transaction).filter(Transaction.account_id == account, Transaction.trade < begin_today).all()
        ...
        time_period = "Period between " + str(one_day_ago).split(".")[0] + " and " + str(current_time).split(".")[
        ...
        positionList = db.session.query(StockPosition).filter(StockPosition.account_id == account,
                                                              StockPosition.date > one_day_ago).all()
        ...
        positionList = db.session.query(StockPosition).filter(StockPosition.account_id == account,
                                                              StockPosition.date > x_days_ago).all()
        ...
        time_period = "Period between " + str(last_month_begin).split(".")[0] + " and " + \
                      str(last_month_end).split(".")[0]
        ...
        positionList = db.session.query(StockPosition).filter(StockPosition.account_id == account,
                                                              StockPosition.date < last_month_begin,
                                                              StockPosition.date > last_month_end).all()
        ...
        positionList = db.session.query(StockPosition).filter(StockPosition.account_id == account,
                                                              StockPosition.date < sub_months).all()
    ...
    if date == "last_year":
        ...
        time_period = "Period between " + str(last_year_begin).split(".")[0] + " and " + str(last_year_end).split(".")[
            0]
        ...
        positionList = db.session.query(StockPosition).filter(StockPosition.account_id == account,
                                                              StockPosition.date < last_year_begin,
                                                              StockPosition.date > last_year_end).all()
